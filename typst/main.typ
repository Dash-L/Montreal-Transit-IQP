...
#align(center + horizon)[
  ...
  #image("images/stm_metro_map.png", fit: "stretch", height: 5in, width: 6in)
...
#set par(first-line-indent: (all: true, amount: 2em), leading: 1.5em, spacing: 1.5em)
...
= Introduction
Today, the majority of the world's population lives in urban areas. While this provides benefits for urban societies and the global climate, it also comes with the challenge of transporting many people in a dense space @ritchie_urbanization_2018. Our project aims to see how one specific city, Montreal, addresses this issue. With over 4 million inhabitants in its metropolitan area, Montreal needs extensive infrastructure to safely and efficiently move people around the city @spieler_trains_2021. Despite its existing public transportation network, 70% of Montrealers primarily commute by car @statistics_canada_main_2024. This reliance on personal vehicles has immense negative effects on urban areas: cars emit high levels of greenhouse gases and air pollutants, reduce safety due to crash risks, and contribute to health problems due to reduced physical activity, all while being inefficient due to high traffic congestion in cities @taillandier_back_2023. Additionally, having car-centric infrastructure leads to land being used by vehicles that will sit idle and parked for the majority of the day, taking up entire areas of densely populated cities. Accommodating these cars also promotes wasteful development, as land zoning will allow for multilane highways, overpasses, and parking garages or lots that serve as collections for heat island effects @ibrahim_impact_2018, making cities less aesthetically pleasing and pedestrian-friendly. Public transit serves as a more sustainable alternative to increasing personal vehicles. The main public transit system in Montreal is the métro, which we will be focusing on for this project. Our research aims to understand the history of the métro and answer the question "What issues currently face the Montreal métro system, and how can it be improved?"

Our project sponsor is the WPI Montreal Project Center. Their objectives are to examine what makes a good transit system, to understand the current dimensions and performance of the Montreal network along with its strengths and challenges, and to highlight key lessons and make recommendations for Montreal and other cities. While not our official sponsor, we will also be working closely with the Société de Transport de Montréal (STM), which is the municipal agency responsible for running Montreal's métro as well as its bus network. They are interested in obtaining a better understanding of the systems they run and what steps could be taken to improve the quality of their services.

Our project aims to explore the effectiveness of the Montreal métro system through many lenses, such as traditional means of success and more abstract values. Traditional success refers to metrics purely about the state of the métro, with things like timeliness, cleanliness, and proximity, whereas abstract means of success are ones riders often ignore, such as sustainability. To achieve this, we will examine its current operations through personal anecdotes and ethnographies of the métro stations and the métro cars, as well as semi-structured interviews and surveys. We will interview planners and representatives from the STM, city officials involved with métro operations, and researchers or professors from local universities. These goals help us focus our project to convey the need for an effective public transit network and provide an objective way to deliver these considerations to relevant stakeholders. To deliver our findings, we will write a technical report and produce a website using ArcGIS mapping tools and data layers.

While the practicality of a good public transit is widely known, the implementation is more difficult to prescribe without a case study and a resulting report with various methods for improving all aspects and states of a city's transit. Montreal is an optimal case study for this problem because of its rank in ridership per capita and affordability compared to other Canadian and American cities. While ranking the 14 most populated cities in 2016, Montreal ranked 12th; however, for ridership per capita, it ranked second to the New York City metropolitan area despite having about 14.5 million fewer people. Montreal happens to achieve this while also having the second most affordable monthly fare, based on hours of work needed at minimum wage, leading to half the hours needed in New York City @verbich_bang_2017. Montreal is able to do this as 41% of the operating budget for the STM comes from transit fares, 34% from the municipal government, and 23% coming from the provincial government @stm_budget_2015. Most cities struggle to find a balance between the income generated from fares and the ability to provide service at an acceptable level, whereas Montreal is able to have the third-highest rider satisfaction index and the second-highest farebox recovery ratio for the transit agency @verbich_bang_2017. Montreal is a clear leader when examining the total value of public transit in North American cities, leading to the crucial need for the case study and report based on the results we found regarding the métro.
...
As the second largest city in Canada and one of the 15 most populated in North America, /*#text(red)[(find a good way to say/cite this, the census data viewer #text(blue)[#link("https://www150.statcan.gc.ca/t1/tbl1/en/cv!recreate.action?pid=9810000201&selectedNodeIds=2D1,2D10,2D11,2D12,2D13&checkedLevels=0D3&refPeriods=20210101,20210101&dimensionLayouts=layout3,layout2,layout2&vectorDisplay=false")[here]] categorizes things in a weird way, that kinda disagrees with wikipedia)]*/ Montreal is an important case study for public transportation in a variety of contexts. Montreal is also unique among otherwise similar areas due to its highly bilingual nature.
...
@rpc_vs_pop_density shows the population density for each city compared to the number of trips taken per capita in the year 2023. Both of these statistics are calculated using the entire metropolitan area for each city (as defined by the US Census), rather than their administrative boundaries. It also aggregates rides from all modes of transit and all transit agencies listed as belonging to each metropolitan area. Our remaining data analysis will only examine the largest transit agency for each city, as determined by the TRAM paper. A list of these transit agencies for each city and its corresponding metropolitan area is provided below.

//This first graph is actually using data about the metropolitan areas which the labeled cities are in. That is, the population density is over the entire metro area, and the rides per capita includes all transit agencies and modes that the NTD considers to serve that metropolitan area. The TRAM paper picks out and analyzes only a specific (likely the largest) transportation agency in each metro area, which we also do for the second graph and likely the rest of our data analysis. The list of metro areas, as labeled in the census data, are provided below, along with the selected transportation agency, as labeled in the NTD data.
...
Our main deliverables will be a report detailing our findings and a set of interactive maps aiming to capture the various aspects of Montreal, along with its metro system. Since the project center is our sponsor, we will not have to worry about getting to and from a sponsor's office, but a lot of our work will be ethnographic field work, where we take the metro to particular stops and neighborhoods.

#figure(caption: [Proposed Gantt chart for our time in Montreal], 
image("images/Gantt Chart.png"))

Our project aims to evaluate Montreal's metro from a qualitative point of view and explain why a good metro is so important for a city like Montreal, especially with reference to social aspects, the environment, physical health, and the economy. In an increasingly urban world, what can other cities learn from Montreal? What is Montreal already doing well, how are they working to improve, and what future steps should they take? By the end of this project, we hope to find and share answers to these questions.
